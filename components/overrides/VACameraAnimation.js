...
const { Potree, THREE, viewer } = window
...
export class CameraAnimation extends Potree.EventDispatcher {
  constructor (viewer) {
    super()
  ...
  static defaultFromView (viewer) {
    ...
    const angle = Potree.computeAzimuth(camera.position, target)

    const n = 5
    for (let i = 0; i < n; i++) {
      const u = 1.5 * Math.PI * (i / n) + angle
