...
export class VAOrientedImageControls extends Potree.EventDispatcher {
  constructor (viewer) {
    super()
    ...
    // const style = ``;
    this.elUp = $(
      '<input type="button" value="🡅" style="position: absolute; top: 10px; left: calc(50%); z-index: 1000" />'
    )
    this.elRight = $(
      '<input type="button" value="🡆" style="position: absolute; top: calc(50%); right: 10px; z-index: 1000" />'
    )
    this.elDown = $(
      '<input type="button" value="🡇" style="position: absolute; bottom: 10px; left: calc(50%); z-index: 1000" />'
    )
    this.elLeft = $(
