// Copyid from the Source code of the VRControls: https://github.com/potree/potree/blob/develop/src/navigation/VRControls.js
...
function computeMove (vrControls, controller) {
  ...
  const rotation = new THREE.Quaternion().setFromEuler(controller.rotation)
...
class TranslationMode {
  ...
  update (vrControls, delta) {
    ...
    const pos = new THREE.Vector3().addVectors(this.startPos, diff)
...
class RotScaleMode {
  ...
  update (vrControls, delta) {
    ...
    const angleStart = new THREE.Vector2(start_c1_c2.x, start_c1_c2.z).angle()
    const angleEnd = new THREE.Vector2(end_c1_c2.x, end_c1_c2.z).angle()
    ...
    const mToOrigin = new THREE.Matrix4().makeTranslation(...toScene(start_center, this.startState).multiplyScalar(-1).toArray())
    const mToStart = new THREE.Matrix4().makeTranslation(...toScene(start_center, this.startState).toArray())
    const mRotate = new THREE.Matrix4().makeRotationZ(angleDiff)
    const mScale = new THREE.Matrix4().makeScale(1 / scale, 1 / scale, 1 / scale)
    ...
    const mToNew = new THREE.Matrix4().makeTranslation(...toNew.toArray())
...
export class VRControls extends EventDispatcher {
  ...
  createSlider (label, min, max) {
    ...
    const matHandle = new THREE.MeshBasicMaterial({ color: 0xFF0000 })
    const matScale = new THREE.MeshBasicMaterial({ color: 0xFF4444 })
    const matValue = new THREE.MeshNormalMaterial()

    const node = new THREE.Object3D('slider')
    const nLabel = new Potree.TextSprite(`${label}: 0`)
    const nMax = new THREE.Mesh(sg, matHandle)
    const nMin = new THREE.Mesh(sg, matHandle)
    const nValue = new THREE.Mesh(sg, matValue)
    const nScale = new THREE.Mesh(cg, matScale)
  ...
  createInfo () {
    const texture = new THREE.TextureLoader().load(`${Potree.resourcePath}/images/vr_controller_help.jpg`)
    const plane = new THREE.PlaneBufferGeometry(1, 1, 1, 1)
    const infoMaterial = new THREE.MeshBasicMaterial({ map: texture })
    const infoNode = new THREE.Mesh(plane, infoMaterial)
  ...
  initMenu (controller) {
    ...
    const node = new THREE.Object3D('vr menu')
  ...
  toScene (vec) {
    ...
    const result = vec.clone().applyMatrix4(mat)
  ...
  setMode (mode) {
    ...
    for (const controller of [this.cPrimary, this.cSecondary]) {
      const start = {
  ...
  getCamera () {
    const reference = this.viewer.scene.getActiveCamera()
